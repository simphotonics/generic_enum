...
**Important**: `@GenerateFromJson()` requires `@GenerateBuiltMap()` since the `fromJson`
function uses the BuiltMap `valueMap` to retrieve generic enum instances based on their value.
