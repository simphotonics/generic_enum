...
- Extend `GenericEnum<T>` where T is Dart built-in type or a class with a const constructor.
- Define a private `const` constructor that calls the super constructor and passes on the value of type <T>.
- Define the static const instances of `DpiResolution`. You may capitalize instance names to mark them as constants.
- Optionally: Create a private field of type `Map<T,EnumClass>` and a corresponding getter
  in order to provide easy access to all values and instances.

```Dart
import 'package:generic_enum/generic_enum.dart';

// Extend GenericEnum<T>
class DpiResolution extends GeneralizedEnum<int> {

  // Define a private const constructor that calls the super constructor
  // and passed on the value of type int.
  const DpiResolution._(int) : super(value);

  // Define static constant instances of DpiResolution
  static const DpiResolution LOW = DpiResolution._(90);
  static const DpiResolution MEDIUM = DpiResolution._(300);
  static const DpiResolution HIGH = DpiResolution._(600);

  // Define a private field mapping each value to its instance.
